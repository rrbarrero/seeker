...
describe("Position Entity", () => {
    ...
    it("should throw error if company is missing", () => {
        ...
        expect(() => Position.fromPrimitives(props)).toThrow("Company name is required");
    });

    it("should throw error if role_title is missing", () => {
        const props = { ...validProps, role_title: " " };
        expect(() => Position.fromPrimitives(props)).toThrow("Role title is required");
    });
    ...
    describe("advanceStatus", () => {
        ...
        it("should throw error for forbidden transitions (e.g. Reject back to CvSent)", () => {
            const position = Position.fromPrimitives({ ...validProps, status: "Rejected" });
            expect(() => position.advanceStatus("CvSent")).toThrow("Cannot transition from Rejected to CvSent");
        });

        it("should throw error for forbidden transitions (e.g. Offer back to CvSent)", () => {
            const position = Position.fromPrimitives({ ...validProps, status: "OfferReceived" });
            expect(() => position.advanceStatus("CvSent")).toThrow("Cannot transition from OfferReceived to CvSent");
        });
    });
});
